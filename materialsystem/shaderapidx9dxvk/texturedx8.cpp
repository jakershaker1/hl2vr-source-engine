...
static ImageFormat GetImageFormat( IDirect3DBaseTexture* pTexture )
{
	...
	if ( pTexture )
	{
		...
		if ( !IsVolumeTexture( pTexture ) )
		{
			...
			if ( !FAILED( hr ) )
				return DxvkFormat::D3DFormatToImageFormat( desc.Format );
		}
		else
		{
			D3DVOLUME_DESC desc;
		...
		ImageFormat dstFormat, int numLevels, int nCreationFlags, char *debugLabel )		// OK to skip the last param
{
	...
	if ( IsX360() )
	{
		...
		if ( !bVertexTexture )
		{
			d3dFormat = DxvkFormat::ImageFormatToD3DFormat( FindNearestSupportedFormat( dstFormat, false, false, false ) );
		}
	}
	else
	{
		d3dFormat = DxvkFormat::ImageFormatToD3DFormat( FindNearestSupportedFormat( dstFormat, bVertexTexture, bIsRenderTarget, bAllowNonFilterable ) );
	}
...
static void BlitSurfaceBits( TextureLoadInfo_t &info, int xOffset, int yOffset, int srcStride )
{
	...
	if ( !info.m_bTextureIsLockable )
	{
		...
		ImageFormat dstFormat = DxvkFormat::D3DFormatToImageFormat( desc.Format );
		D3DFORMAT dstFormatD3D = DxvkFormat::ImageFormatToD3DFormat( dstFormat );
...
int ComputeTextureMemorySize( const GUID &nDeviceGUID, D3DDEVTYPE deviceType )
{
	...
	while (true)
	{
		...
		RECORD_INT( 256 );
		RECORD_INT( 256 );
		RECORD_INT( DxvkFormat::ImageFormatToD3DFormat(fmt) );
		RECORD_INT( 1 );
		RECORD_INT( false );
		RECORD_INT( 1 );
