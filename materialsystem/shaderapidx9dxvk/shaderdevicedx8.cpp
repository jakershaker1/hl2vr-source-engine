...
bool CShaderDeviceMgrDx8::Connect( CreateInterfaceFn factory )
{
...
#if defined( DXVK_NATIVE )
	// Must happen before ANY Direct3DCreate9()/D3D9 call below - DXVK's SDL2
	// WSI backend (used for its own Vulkan instance/surface creation) needs
	// SDL_INIT_VIDEO already active, which this also performs. Moving this
	...
	mat_supports_d3d9ex.SetValue( bD3D9ExAvailable ? 1 : 0 );
	...
	#else
		#if defined( DXVK_NATIVE )
		// DXVK's D3D9 entry points throw dxvk::DxvkError (a std::exception)
		// directly out of Direct3DCreate9() on init failure, rather than
		// converting to a null return - catch it here (this is the actual
		// call site; a try/catch anywhere later in Connect() is too late).
		Warning( "CShaderDeviceMgrDx8::Connect: DXVK_WSI_DRIVER=%s\n", getenv( "DXVK_WSI_DRIVER" ) ? getenv( "DXVK_WSI_DRIVER" ) : "(unset)" );
		try
		{
			m_pD3D = Direct3DCreate9(D3D_SDK_VERSION);
		}
		...
		{
			Warning( "CShaderDeviceMgrDx8::Connect: Direct3DCreate9 threw: %s\n", e.what() );
			m_pD3D = NULL;
		}
		catch ( ... )
		{
			Warning( "CShaderDeviceMgrDx8::Connect: Direct3DCreate9 threw an unknown exception\n" );
			m_pD3D = NULL;
		}
		#else
		m_pD3D = Direct3DCreate9(D3D_SDK_VERSION);
...
#if defined( DXVK_NATIVE )
	// This Android build's engine init flow never calls the normal
	// materials->SetMode(hwnd, config) entry point that would normally
	// create the real device (there's no desktop window to get an hwnd
	// from) - confirmed by repo-wide search, zero call sites outside this
	// module. Unlike shaderapivulkan (a from-scratch stub, so it could
	// safely defer device creation to first Present() and guard the few
	// spots that ran before that), this is the real, full shaderapidx9
	// renderer: dozens of call sites throughout this codebase dereference
	// Dx9Device() with no null checks, so the device needs to exist before
	// any of that runs. Connect() is the earliest point we have a reliable
	// hook (standard IAppSystem lifecycle, always called) and doesn't need
	// anything OpenXR-related yet - DXVK's device only needs a window.
	Warning( "CShaderDeviceMgrDx8::Connect: DXVK_WSI_DRIVER=%s\n", getenv( "DXVK_WSI_DRIVER" ) ? getenv( "DXVK_WSI_DRIVER" ) : "(unset)" );
	VD3DHWND hDxvkWnd = DXVK_GetOrCreateWindow();
	if ( hDxvkWnd )
	{
		InitAdapterInfo();

		ShaderDeviceInfo_t info;
		info.m_DisplayMode.m_nWidth = 1280;
		info.m_DisplayMode.m_nHeight = 720;
		info.m_DisplayMode.m_Format = IMAGE_FORMAT_BGRA8888;
		info.m_nBackBufferCount = 1;
		info.m_bWindowed = true;
		info.m_bWaitForVSync = false;

		// DXVK's own D3D9 entry points throw dxvk::DxvkError (a
		// std::exception) internally on failure rather than always
		// converting to an HRESULT - catch it here so a real Vulkan/DXVK
		// init failure prints a readable message instead of an unhandled
		// std::terminate() crash with no diagnostic.
		try
		{
			if ( !SetMode( (void*)hDxvkWnd, 0, info ) )
			{
				Warning( "CShaderDeviceMgrDx8::Connect: SetMode failed - DXVK device not created\n" );
			}
		}
		catch ( const std::exception &e )
		{
			Warning( "CShaderDeviceMgrDx8::Connect: DXVK threw during device creation: %s\n", e.what() );
		}
		catch ( ... )
		{
			Warning( "CShaderDeviceMgrDx8::Connect: DXVK threw an unknown exception during device creation\n" );
		}
	}
	else
	{
		Warning( "CShaderDeviceMgrDx8::Connect: DXVK_GetOrCreateWindow failed - no D3D9 device will be created\n" );
	}
#endif
...
void CShaderDeviceMgrDx8::GetModeInfo( ShaderDisplayMode_t* pInfo, int nAdapter, int nMode ) const
{
	...
	pInfo->m_Format      = DxvkFormat::D3DFormatToImageFormat( d3dInfo.Format );
	pInfo->m_nRefreshRateNumerator = d3dInfo.RefreshRate;
	pInfo->m_nRefreshRateDenominator = 1;
#else
	pInfo->m_Format = DxvkFormat::D3DFormatToImageFormat( D3DFMT_X8R8G8B8 );
...
void CShaderDeviceMgrDx8::GetCurrentModeInfo( ShaderDisplayMode_t* pInfo, int nAdapter ) const
{
	...
	pInfo->m_Format = DxvkFormat::D3DFormatToImageFormat( mode.Format );
...
int CShaderDeviceMgrDx8::GetVidMemBytes( int nAdapter ) const
{
	...
	// FIXME: no WMI on Android to query this (that's the win32-only path
	// below, materialsystem/shaderapidx9/wmi.cpp - not built for this
	// module). Stub a plausible mobile-GPU value; revisit via a real
	// Vulkan VkPhysicalDeviceMemoryProperties query if this ever needs to
	// be accurate (e.g. for texture budget decisions).
...
void CShaderDeviceDx8::SetPresentParameters( void* hWnd, int nAdapter, const ShaderDeviceInfo_t &info )
{
	...
	Warning( "SetPresentParameters: m_AdapterFormat=%d requestedFormat=%d mode.m_Format=%d mode.w=%d mode.h=%d\n",
		(int)m_AdapterFormat, (int)info.m_DisplayMode.m_Format, (int)mode.m_Format, mode.m_nWidth, mode.m_nHeight );
	ImageFormat backBufferFormat = FindNearestSupportedBackBufferFormat( nAdapter,
		DX8_DEVTYPE, m_AdapterFormat, info.m_DisplayMode.m_Format, info.m_bWindowed );
	Warning( "SetPresentParameters: FindNearestSupportedBackBufferFormat returned %d\n", (int)backBufferFormat );
	...
	if ( IsX360() || !info.m_bWindowed )
	{
		...
		m_PresentParameters.BackBufferFormat = DxvkFormat::ImageFormatToD3DFormat( backBufferFormat );
#if defined( _X360 )
	...
	else
	{
		...
		m_PresentParameters.BackBufferFormat = DxvkFormat::ImageFormatToD3DFormat( backBufferFormat );
		m_PresentParameters.BackBufferCount = 1;
		Warning( "SetPresentParameters: windowed branch set BackBufferFormat=%d (from backBufferFormat=%d)\n",
			(int)m_PresentParameters.BackBufferFormat, (int)backBufferFormat );
	}
...
{
	return DxvkFormat::D3DFormatToImageFormat( m_PresentParameters.BackBufferFormat );
}
...
#if defined( DXVK_NATIVE )
		DXVK_PresentFrameToXR( Dx9Device() );
#endif
	}
