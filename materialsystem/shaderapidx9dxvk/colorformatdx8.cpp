							   ...
							   bool bIsVertexTexture, bool bIsFilterableRequired ) 
{
										   ...
										   ( format == ATIFMT_D16 ) || ( format == ATIFMT_D24S8 ) ) )
	{
		hr = D3D()->CheckDeviceFormat(
			g_DisplayAdapter, g_DeviceType, DxvkFormat::ImageFormatToD3DFormat( g_DeviceFormat ),
...
{
	return DxvkFormat::D3DFormatToImageFormat( g_D3DColorFormat[format][bIsVertexTexture][bIsRenderTarget][bFilterableRequired] );
}
...
{
	D3DFORMAT d3dDisplayFormat = DxvkFormat::ImageFormatToD3DFormat( displayFormat );
	D3DFORMAT d3dRenderTargetFormat = DxvkFormat::ImageFormatToD3DFormat( renderTargetFormat );
