...
void CShaderAPIDx8::AcquireInternalRenderTargets()
{
	...
	if ( !m_pBackBufferSurfaceSRGB )
	{
		...
		D3DFORMAT backBufferFormat = DxvkFormat::ImageFormatToD3DFormat( g_pShaderDevice->GetBackBufferFormat() );
...
//	imageFormat = DxvkFormat::D3DFormatToImageFormat( desc.Format );
//	if( imageFormat < 0 )
//	{
//		Assert( 0 );
//		return;
...
void CShaderAPIDx8::ExportTextureList()
{
	...
	//	ImageFormat imageFormat = DxvkFormat::D3DFormatToImageFormat( desc.Format );
	//	if( imageFormat >= 0 )
		{
	...
	for ( ShaderAPITextureHandle_t hTexture = m_Textures.Head() ; hTexture != m_Textures.InvalidIndex(); hTexture = m_Textures.Next( hTexture ) )
	{
		...
		pXTextureList[numTextures].pFormatName = D3DFormatName( DxvkFormat::ImageFormatToD3DFormat( tex.GetImageFormat() ) );
	...
	int size = ImageLoader::GetMemRequired( 
		...
		desc.Height,
		0,
		DxvkFormat::D3DFormatToImageFormat( desc.Format ),
		false );
	...
	const char *pTextureGroupName )
{
	...
	for ( int idxFrame = 0; idxFrame < count; ++ idxFrame )
	{
		...
		RECORD_INT( DxvkFormat::ImageFormatToD3DFormat( FindNearestSupportedFormat(dstImageFormat)) );
...
//				MEM_ALLOC_CREDIT();
		...
		if ( pD3DTex )
		{
			...
			dstImageFormat = DxvkFormat::D3DFormatToImageFormat( desc.Format );
...
IDirect3DSurface* CShaderAPIDx8::GetFrontBufferImage( ImageFormat& format )
{
	...
	format = DxvkFormat::D3DFormatToImageFormat( D3DFMT_A8R8G8B8 );
	return pSurfaceBits;
#else
	Assert( 0 );
	return NULL;
...
IDirect3DSurface* CShaderAPIDx8::GetBackBufferImageHDR( Rect_t *pSrcRect, Rect_t *pDstRect, ImageFormat& format )
{
	...
	format = DxvkFormat::D3DFormatToImageFormat(desc.Format);
	if ( pTmpSurface )
	{
		pTmpSurface->Release();
	}
...
ShaderAPITextureHandle_t CShaderAPIDx8::CreateRenderTargetSurface( int width, int height, ImageFormat format, const char *pDebugName, const char *pTextureGroupName )
{
	...
	D3DFORMAT actualFormat = DxvkFormat::ImageFormatToD3DFormat( dstImageFormat );
